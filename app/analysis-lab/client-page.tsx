...
export default function Dashboard() {
  ...
  const handleDownloadPDF = (report: ResumeReport) => {
    if (!report.feedback?.improved_resume) return;

    const doc = new jsPDF();
    const margin = 20;
    const pageWidth = doc.internal.pageSize.getWidth();
    const text = report.feedback.improved_resume;
    
    doc.setFont("helvetica", "bold");
    doc.setFontSize(22);
    doc.text("Optimized Resume", margin, 30);
    
    doc.setFont("helvetica", "normal");
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`Generated by ResuAI on ${new Date(report.createdAt).toLocaleDateString()}`, margin, 38);
    ...
    doc.save(`ResuAI_Optimized_Resume_${new Date(report.createdAt).getTime()}.pdf`);
  };
                ...
                >
                  <div className="flex flex-col gap-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                      ...
                      {report.feedback?.improved_resume && (
                        <button
                          onClick={() => handleDownloadPDF(report)}
                          className="flex items-center gap-2 px-5 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold transition-all shadow-lg shadow-indigo-500/20 active:scale-95"
                        >
                          <Download size={18} />
                    ...
                    {report.feedback?.improved_resume && (
                      <div className="mt-4 p-6 rounded-2xl bg-white/[0.03] border border-white/5">
                        ...
                        <pre className="text-zinc-400 text-xs md:text-sm leading-relaxed whitespace-pre-wrap font-mono bg-black/20 p-4 rounded-xl border border-white/5">
                          {report.feedback.improved_resume}
                        </pre>
                      </div>
                    )}
